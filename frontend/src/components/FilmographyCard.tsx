...
import { FilmographyItem } from "@/lib/types";

export default function FilmographyCard({ item }: { item: FilmographyItem }) {
  const poster = posterUrl(item.poster_path, "w500");
  const year = item.release_date ? item.release_date.slice(0, 4) : "TBA";
  ...
  return (
    ...
    >
      <div className="relative aspect-[2/3] w-full bg-zinc-100 dark:bg-zinc-800">
        {poster ? (
          <Image src={poster} alt={item.title} fill sizes="(max-width: 640px) 45vw, 185px" className="object-cover" />
        ) : (
